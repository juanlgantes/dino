...
export class App {
  ...
  initDOM() {
    ...
    document.getElementById("btnToggleCinema").onclick = () =>
      this.toggleCinemaLock();
  ...
  renderActivities(mode = "root") {
    ...
    Object.keys(ACTIVITIES_DATA).forEach((key) => {
      ...
      if (
        (mode === "root" || mode === "arcade") &&
        (isEnglish || isComunicacion)
      )
        return;
  ...
  startGame(key) {
    ...
    if (data.class) {
      if (
        ["connect_four", "checkers", "parchis", "chess", "goose"].includes(
          data.type,
        )
      ) {
  ...
  saveMagicWord() {
    ...
    if (val) {
      ...
      document.getElementById("magicMsg").textContent =
        "🔓 Acceso libre activado";
    }
    this.persistState();
  }
  ...
  resetProgress() {
    if (
      confirm("¿Seguro que quieres borrar TODO el progreso? (Estrellas a 0)")
    ) {
      this.state = {
        ...this.state,
        score: 0,
  ...
  updateParentUI() {
    ...
    if (btn) {
      btn.textContent = this.state.cinemaLocked
        ? "🔓 Desbloquear Cine"
        : "🔒 Bloquear Cine";
  ...
  renderCinema() {
    ...
    const unlockedCount = this.state.cinemaLocked
      ? 0
      : Math.floor(this.state.totalStars / 100);
