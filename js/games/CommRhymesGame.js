export class CommRhymesGame {
    constructor(data, container) {
        ...
        this.c.style.background = 'linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)';
    ...
    checkAnswer(opt, btn) {
        ...
        if (opt.correct) {
            ...
            this.textEl.textContent = this.rhymes[this.currentRhymeIndex].text + " " + opt.word.toUpperCase() + "!";
            if (window.app.audio && window.app.audio.speak) {
                window.app.audio.speak(opt.word, 'es-ES');
            }

            this.score += 5;
            document.getElementById('rhymeScore').textContent = 'Estrellas: ' + this.score;
