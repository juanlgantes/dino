export class CommInstrumentsGame {
    ...
    spawnInstrument() {
        ...
        const itemObj = { el, x, y, speed: 1 + Math.random(), type: inst.type, dragging: false };
    ...
    checkDrop(item) {
        ...
        baskets.forEach(b => {
            ...
            // Collision detection
            if (itemRect.left < bRect.right && itemRect.right > bRect.left &&
                itemRect.top < bRect.bottom && itemRect.bottom > bRect.top) {

                if (item.type === b.dataset.type) {
                    // Correct!
                    window.app.audio.playPop();
                    this.score++;
                    document.getElementById('instScore').textContent = 'Score: ' + this.score;
    ...
    endGame() {
        ...
        document.getElementById('btnReplayInst').onclick = () => window.app.startGame(window.app.currentGameKey);
    }
