export class RecycleGame {
  ...
  init() {
    ...
    h.style.cssText =
      "position:absolute; top:20%; width:100%; text-align:center; font-size:3em; color:#2d3436;";
    ...
    btn.style.cssText =
      "position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:#00b894;";
  ...
  startGame() {
    ...
    hud.style.cssText =
      "position:absolute; top:10px; left:10px; font-size:1.5em; font-weight:bold; color:#2d3436; background:rgba(255,255,255,0.8); padding:5px 15px; border-radius:15px;";
    hud.id = "recycleHud";
    ...
    binCont.style.cssText =
      "position:absolute; bottom:0; left:0; width:100%; height:120px; display:flex;";
    ...
    this.timerInterval = setInterval(() => {
      ...
      this.timeLeft--;
      document.getElementById("recycleHud").textContent =
        `⏱️ ${this.timeLeft}s`;
  ...
  spawnItem() {
    ...
    const itemIcon =
      binType.types[Math.floor(Math.random() * binType.types.length)];
    ...
    const item = {
      ...
      type: binType.id,
      y: -50,
      speed: Math.random() * 2 + 1 + (60 - this.timeLeft) / 30,
      dragging: false,
    };
