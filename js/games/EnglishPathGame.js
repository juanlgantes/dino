export class EnglishPathGame {
    constructor(data, container) {
        ...
        // Setup UI
        this.c.style.background = 'linear-gradient(to bottom, #84fab0 0%, #8fd3f4 100%)';
    ...
    initMenu() {
        ...
        document.getElementById('btnNums').onclick = () => this.startGame('numbers');
        document.getElementById('btnMonths').onclick = () => this.startGame('months');
    }

    startGame(mode) {
        if (mode === 'numbers') {
            for (let i = 1; i <= 25; i++) this.sequence.push({ label: i.toString(), spoken: i.toString() });
        } else {
            const months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
            const icons = ['❄️', '💘', '🌱', '🌧️', '🌸', '☀️', '🏖️', '🍉', '🍂', '🎃', '🦃', '🎄'];
            months.forEach((m, i) => this.sequence.push({ label: m, icon: icons[i], spoken: m }));
        }
    ...
    updateTargetUI() {
        if (this.currentIndex < this.sequence.length) {
            document.getElementById('nextLabel').textContent = this.sequence[this.currentIndex].label;
            if (window.app.audio && window.app.audio.speak) {
                window.app.audio.speak(this.sequence[this.currentIndex].spoken, 'en-US');
            }
        }
    }
    ...
    spawnBatch() {
        ...
        const itemsNeeded = 5;
        let spawned = 0;
        for (let i = this.currentIndex; i < this.sequence.length && spawned < itemsNeeded; i++) {
    ...
    hitItem(item) {
        if (item.index === this.currentIndex) {
            ...
            if (!this.errorCooldown) {
                ...
                setTimeout(() => { this.errorCooldown = false; }, 1000);
            }
        }
    }
