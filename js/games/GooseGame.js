export class GooseGame {
  ...
  init() {
    ...
    title.style.cssText =
      "color:#2c3e50; font-size:3em; margin-bottom:30px; text-align:center;";
    ...
    btn3.innerHTML =
      '🎲 Híbrido<br><span style="font-size:0.6em">Dado Real</span>';
  ...
  renderSpiral() {
    ...
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        ...
        // Iconos Grandes
        if (this.DINOS.includes(val)) {
          cell.style.background = "#f1c40f";
          cell.innerHTML += '<div style="font-size:1.8em">🦕</div>';
        } else if (this.SPECIALS[val]) {
          cell.style.background = "#e74c3c";
          cell.style.color = "white";
          cell.innerHTML += '<div style="font-size:1.8em">⚠️</div>';
        } else if (val === 63) {
          cell.style.background = "#2ecc71";
          cell.innerHTML = '<div style="font-size:2em">🥚</div>';
        }
  ...
  createTokens() {
    ...
    this.pieces.forEach((_, i) => {
      ...
      t.innerText = icons[i] || "👤";
      t.style.cssText =
        "position:absolute; font-size:2.2em; transition:all 0.5s; z-index:20; pointer-events:none; filter: drop-shadow(0 0 2px white);";
    ...
    this.tokens.forEach((token, idx) => {
      ...
      if (cell) {
        let x =
          cell.offsetLeft +
          cell.offsetWidth / 2 -
          (token.offsetWidth / 2 || 15);
        let y =
          cell.offsetTop +
          cell.offsetHeight / 2 -
          (token.offsetHeight / 2 || 15);
