export class ParchisGame {
  ...
  init() {
    ...
    btn4.innerHTML =
      '👥 4 Jugadores<br><span style="font-size:0.6em">Fiesta</span>';
    ...
    btn3.innerHTML =
      '🎲 Híbrido<br><span style="font-size:0.6em">2P Real</span>';
    ...
    btn5.innerHTML =
      '🎲 Híbrido 4<br><span style="font-size:0.6em">4P Real</span>';
    ...
    if (this.savedState) {
      ...
      btnResume.innerHTML =
        '📂 Continuar<br><span style="font-size:0.6em">Partida Guardada</span>';
  ...
  resumeGame() {
    if (this.savedState) {
      ...
      this.msg =
        this.gameMode === "hybrid" || this.gameMode === "hybrid4"
          ? "Tira tu dado y anota 📝"
          : "Tira el dado 🎲";
      this.setupBoard();
      this.updateTurnUI();
  ...
  setupBoard() {
    ...
    header.style.cssText =
      "display:flex; justify-content:space-between; width:100%; max-width:600px; padding:10px; align-items:center;";
    ...
    this.diceEl.style.cssText =
      "font-size:3em; cursor:pointer; background:white; padding:10px; border-radius:10px; box-shadow:0 4px 8px rgba(0,0,0,0.2); user-select:none;";
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      b.textContent = i;
      b.style.cssText =
        "width:50px; height:50px; font-size:1.5em; border:none; border-radius:50%; background:#3498db; color:white; cursor:pointer;";
    ...
    const totalPieces =
      this.gameMode === "pvp4" || this.gameMode === "hybrid4" ? 16 : 8; // 8 for 2P modes
  ...
  renderBoard(container) {
    ...
    for (let r = 0; r < 11; r++) {
      for (let c = 0; c < 11; c++) {
        ...
        // Corners (Homes)
        if (r < 4 && c < 4)
          div.style.background = "#e74c3c"; // Red Home
        else if (r < 4 && c > 6)
          div.style.background = "#f1c40f"; // Yellow Home
        else if (r > 6 && c < 4)
          div.style.background = "#3498db"; // Blue (Unused in 2P but shown)
        else if (r > 6 && c > 6)
          div.style.background = "#2ecc71"; // Green (Unused in 2P but shown)
        // Center
        else if (r === 5 && c === 5)
          div.style.background =
            "conic-gradient(#e74c3c, #f1c40f, #2ecc71, #3498db)";
  ...
  nextTurn() {
    const maxP =
      this.gameMode === "pvp4" || this.gameMode === "hybrid4" ? 4 : 2;
